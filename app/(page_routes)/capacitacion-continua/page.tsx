...
import { Award, BookOpen, CheckCircle2, GraduationCap, RefreshCw } from "lucide-react";
import { useLanguage } from "@/app/context/LanguageContext";
import { translations } from "@/lib/translations";

export default function CapacitacionContinuaPage() {
  const { lang } = useLanguage();
  const tr = translations[lang].capacitacionContinua;
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <div className="mt-5 rounded-xl bg-emerald-50 p-4 dark:bg-emerald-900/20">
              ...
              <p className="mt-1 text-emerald-800 dark:text-emerald-200">{tr.frequency}</p>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
            <h2 className="mb-3 flex items-center gap-2 text-xl font-semibold text-zinc-900 dark:text-white">
              ...
              {tr.certificationTitle}
            </h2>
            <p className="leading-relaxed text-zinc-700 dark:text-zinc-300">{tr.certificationText}</p>
            <div className="mt-5 flex items-center gap-3 rounded-xl bg-zinc-50 p-4 dark:bg-zinc-900">
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                {lang === "es" ? "Directrices del ICT aplicadas" : "ICT guidelines applied"}
              </p>
            </div>
          </section>
        </div>
