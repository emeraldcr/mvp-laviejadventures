...
import { CheckCircle2, Globe, Megaphone, MessageCircle, TrendingUp } from "lucide-react";
import { useLanguage } from "@/app/context/LanguageContext";
import { translations } from "@/lib/translations";

export default function PlanMercadeoPage() {
  const { lang } = useLanguage();
  const tr = translations[lang].planMercadeo;
      ...
      <div className="mx-auto w-full max-w-4xl space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <ul className="space-y-3">
              {tr.experiences.map((experience, i) => (
                <li key={i} className="flex items-start gap-3 text-zinc-700 dark:text-zinc-300">
                  <CheckCircle2 size={18} className="mt-0.5 shrink-0 text-emerald-500" />
        ...
        <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950 md:p-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {tr.channels.map((channel, i) => (
              <div
                key={i}
                className="rounded-xl bg-zinc-50 px-4 py-3 text-zinc-700 dark:bg-zinc-900 dark:text-zinc-300"
              >
                {channel}
              </div>
            ))}
