...
const categories = [
  { number: "01", title: "Cuerdas", subtitle: "Guitarras · Bajos · Accesorios", icon: Guitar, color: "#3778ff" },
  { number: "02", title: "Teclas & vientos", subtitle: "Pianos · Teclados · Vientos", icon: Piano, color: "#21d9e5" },
  { number: "03", title: "Percusión", subtitle: "Baterías · Ritmo · Accesorios", icon: Drum, color: "#8de65c" },
  { number: "04", title: "Audio profesional", subtitle: "Amplis · Micros · Monitoreo", icon: Volume2, color: "#d8ef55" },
];
...
const eventServices = [
  { title: "Sonido profesional", text: "Sistemas de audio para salones, exteriores y escenarios." },
  { title: "Iluminación", text: "Diseño de luces para ambiente, show y producción." },
  { title: "Video & AV", text: "Apoyo audiovisual para presentaciones y espectáculos." },
  { title: "Tarimas y montaje", text: "Infraestructura y coordinación técnica del evento." },
  { title: "DJ / discomóvil", text: "Entretenimiento y transición entre momentos del programa." },
  { title: "Producción integral", text: "Preproducción, montaje y operación el día del evento." },
];
    ...
    <main className="jess-site min-h-screen overflow-clip bg-[#050711] text-white selection:bg-[#a5ef5a] selection:text-[#071025]">
      ...
      <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled ? "border-b border-white/[.08] bg-[#050711]/80 backdrop-blur-2xl" : "bg-transparent"}`}>
        <div className="mx-auto flex h-[76px] max-w-[1680px] items-center justify-between px-5 sm:px-8 lg:px-12">
          <a href="#inicio" aria-label="JESS Music, inicio" className="group relative z-10">
            <img src="/propuestas/grupo-jess/logo.svg" alt="JESS Music & Sound Systems" className="h-10 w-auto drop-shadow-[0_0_18px_rgba(37,219,230,.2)] sm:h-12" />
          </a>
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="grid h-11 w-11 place-items-center rounded-full border border-white/15 bg-black/20 xl:hidden" aria-label="Abrir menú">
            {menuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
          </button>
        </div>
        {menuOpen && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="border-t border-white/10 bg-[#050711]/95 px-5 py-6 backdrop-blur-2xl xl:hidden">
            {NAV.map(([href, label]) => (
      ...
      <section ref={heroRef} id="inicio" className="relative min-h-[100svh] overflow-hidden">
        <motion.div className="absolute inset-0" style={{ y: heroY, scale: heroScale }}>
          <img src="/propuestas/grupo-jess/hero-cinematic-v2.png" alt="Escenario JESS con instrumentos y equipo de audio" className="h-full w-full object-cover object-[67%_center]" />
        </motion.div>
          ...
          <div className="grid items-end gap-10 lg:grid-cols-[minmax(0,1fr)_300px]">
            <motion.div initial="hidden" animate="show" variants={stagger}>
              ...
              <motion.div variants={fadeUp} className="mt-8 flex max-w-3xl flex-col gap-7 border-l border-[#35dfe8]/60 pl-5 sm:flex-row sm:items-end sm:justify-between sm:pl-7">
                ...
                <a href="#grupo" className="group inline-flex shrink-0 items-center gap-3 text-xs font-black uppercase tracking-[.18em]">Descubrir JESS <span className="grid h-11 w-11 place-items-center rounded-full border border-white/25 transition group-hover:border-[#a9eb58] group-hover:bg-[#a9eb58] group-hover:text-black"><ArrowDown className="h-4 w-4" /></span></a>
              </motion.div>
            </motion.div>
            <motion.aside initial={{ opacity: 0, x: 25 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: .7, duration: .8 }} className="hidden border-l border-white/15 pl-8 lg:block">
              ...
              <div className="mt-8 grid grid-cols-2 gap-x-4 gap-y-4 text-[10px] font-bold uppercase tracking-[.14em] text-white/45">
      ...
      <div className="relative z-10 overflow-hidden border-y border-white/10 bg-[#090d1b] py-4">
        <div className="jess-marquee flex w-max items-center whitespace-nowrap text-[10px] font-black uppercase tracking-[.3em] text-white/45">
          {[...Array(2)].flatMap((_, n) => ["Instrumentos", "Audio profesional", "Academia eXpression", "Sound Systems", "Jess Band", "Grabación", "Servicio técnico"].map((item) => (
            <span key={`${n}-${item}`} className="flex items-center"><span className="mx-8 h-1.5 w-1.5 rounded-full bg-[#75e36d]" />{item}</span>
          )))}
        </div>
      </div>

      <section className="relative px-5 py-24 sm:px-8 lg:px-12 lg:py-40">
      ...
      <section id="tienda" className="relative border-t border-white/10 px-5 py-24 sm:px-8 lg:px-12 lg:py-32">
        ...
        <div className="relative mx-auto max-w-[1500px]">
          ...
          />
          <div className="mt-16 grid overflow-hidden rounded-[2rem] border border-white/10 md:grid-cols-2 xl:grid-cols-4">
            {categories.map((item, index) => (
              <motion.article key={item.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ delay: index * .08 }} className="jess-category group relative min-h-[390px] overflow-hidden border-b border-white/10 p-7 last:border-0 md:odd:border-r xl:border-b-0 xl:border-r xl:last:border-r-0 sm:p-9">
                <div className="absolute inset-0 opacity-0 transition duration-700 group-hover:opacity-100" style={{ background: `radial-gradient(circle at 20% 100%, ${item.color}2f, transparent 60%)` }} />
                <div className="relative flex h-full flex-col justify-between">
                  <div className="flex items-start justify-between"><span className="font-mono text-[10px] text-white/30">/{item.number}</span><item.icon className="h-7 w-7 transition duration-500 group-hover:scale-110" style={{ color: item.color }} /></div>
                  <div>
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-[.18em] text-white/35">{item.subtitle}</p>
                    <h3 className="mt-4 text-3xl font-black tracking-[-.04em]">{item.title}</h3>
                    <a href={WHATSAPP} target="_blank" rel="noreferrer" className="mt-8 inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[.2em] text-white/50 transition hover:text-white">Consultar <ArrowUpRight className="h-4 w-4" /></a>
                  </div>
                </div>
              </motion.article>
            ))}
      ...
      <section className="px-5 py-12 sm:px-8 lg:px-12 lg:py-24">
        <div className="mx-auto grid max-w-[1500px] overflow-hidden rounded-[2.5rem] border border-[#2a79ff]/30 bg-[#070c19] shadow-[0_40px_120px_rgba(0,0,0,.45)] lg:grid-cols-[1.08fr_.92fr]">
          <div className="relative min-h-[520px] overflow-hidden lg:min-h-[700px]">
            <img src="/propuestas/grupo-jess/bateria-junior-featured.png" alt="Batería junior de cinco piezas" className="absolute inset-0 h-full w-full object-cover object-center transition duration-[1.5s] hover:scale-[1.03]" />
          ...
          <div className="mt-16 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            {timeline.map((item, index) => (
              <motion.article
                ...
                className="relative overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/[.03] p-6"
              >
                <span className="font-mono text-xs text-[#36dfe7]">{item.year}</span>
        ...
        <div className="mx-auto grid max-w-[1500px] gap-16 lg:grid-cols-[.72fr_1.28fr] lg:gap-24">
          ...
          <div>
            {units.map((unit, index) => (
              <motion.article key={unit.name} initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: .65, delay: index * .04 }} className="group grid gap-7 border-t border-white/12 py-10 first:border-[#2877ff]/50 sm:grid-cols-[70px_1fr_auto] sm:items-start sm:py-12">
                <span className="font-mono text-xs text-white/25">{unit.number}</span>
                <div>
        ...
        <div className="relative mx-auto max-w-[1500px]">
          <div className="grid gap-14 lg:grid-cols-2 lg:items-center">
            ...
            <div className="relative mx-auto aspect-square w-full max-w-[620px]">
              <div className="jess-orbit absolute inset-[10%] rounded-full border border-dashed border-white/15" />
              <div className="absolute inset-[24%] grid place-items-center rounded-full border border-[#2bdce6]/25 bg-[#0a1022] shadow-[0_0_100px_rgba(31,112,255,.22)]">
                <Mic2 className="h-20 w-20 text-[#30dfe8]" />
              </div>
              {[{ t: "Bodas", p: "left-[2%] top-[42%]" }, { t: "Corporativos", p: "right-[0] top-[28%]" }, { t: "Conciertos", p: "bottom-[10%] right-[18%]" }, { t: "Culturales", p: "left-[20%] top-[5%]" }].map((item) => (
                <span key={item.t} className={`absolute ${item.p} rounded-full border border-white/12 bg-[#080c19]/85 px-4 py-2 text-[10px] font-black uppercase tracking-[.16em] text-white/60 backdrop-blur`}>{item.t}</span>
              ))}
            </div>
          </div>
          ...
          <div className="mt-16 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {eventServices.map((service, index) => (
              <motion.article
                ...
                className="rounded-[1.35rem] border border-white/10 bg-white/[.03] p-6"
              >
      ...
      <footer className="border-t border-white/10 bg-[#03050b] px-5 py-10 sm:px-8 lg:px-12">
        <div className="mx-auto flex max-w-[1500px] flex-col justify-between gap-8 sm:flex-row sm:items-end">
          <div>
            <img src="/propuestas/grupo-jess/logo.svg" alt="JESS Music" className="h-14 w-auto" />
