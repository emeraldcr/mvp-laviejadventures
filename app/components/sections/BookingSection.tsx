...
export default function BookingSection({ selectedTourSlug, initialPackageId }: Props) {
  ...
  return (
    <section id="booking" className="relative overflow-hidden bg-[#f4f1ea] pt-6">
      <div className="relative z-10 container mx-auto px-3 py-5 md:px-8 md:py-8">
        <div className="mx-auto mb-6 max-w-5xl rounded-2xl border border-stone-200 bg-white px-5 py-5 shadow-sm sm:px-6">
        ...
        {selectedTourSlug ? (
          <div className="mx-auto grid max-w-[1560px] items-start gap-5 xl:gap-6 lg:grid-cols-[minmax(560px,0.47fr)_minmax(0,0.53fr)]">
            <div className="rounded-2xl border border-stone-200 bg-white pb-4 shadow-sm lg:sticky lg:top-20">
              <CalendarSection />
            </div>
          ...
          <div className="mx-auto max-w-2xl rounded-2xl border border-emerald-200 bg-emerald-50 px-6 py-8 text-center">
...
function ReservationFallback() {
  return (
    <div className="flex min-h-[260px] items-center justify-center rounded-2xl border border-stone-200 bg-stone-50 p-6 text-center">
      <div>
        <div className="mx-auto mb-4 h-10 w-10 animate-spin rounded-full border-2 border-emerald-200 border-t-emerald-600" />
        <p className="text-sm font-bold text-stone-600">Preparando la reserva...</p>
      </div>
    </div>
  );
}
