/**
 * Seed: preguntas estilo prop-bet para IR Iran vs New Zealand (Match #15)
 * Grupo G · Los Angeles Stadium · 2026-06-15T19:00:00-06:00
 *
 *   node --env-file=.env scripts/seed-mundial-iran-newzealand.mjs
 */
...
const MATCH_ID = "world-cup-2026-match-015";
const MATCH_NUMBER = 15;
const MATCH_LABEL = "IR Iran vs New Zealand";
...
const QUESTIONS = [
  ...
  makeQuestion(
    ...
    ["Iran", "New Zealand", "Sin goles"]
  ),

  makeQuestion(
    "¿Quién va ganando al descanso (min 45)?",
    ["Iran", "Empate", "New Zealand"]
  ),

  makeQuestion(
    "¿Iran gana sin recibir gol? (Clean Sheet)",
    ["Sí", "No"]
  ),

  makeQuestion(
    "¿Iran gana con más de 1 gol de diferencia? (Iran -1.5)",
    ["Sí", "No"]
  ),

  makeQuestion(
    "¿Cuántos goles marca New Zealand en total?",
    ["0", "1", "2 o más"]
  ),
  ...
  // ── Jugadores Iran ────────────────────────────────────────────────────────
  makeQuestion(
    "¿Mehdi Taremi anota en el partido?",
    ["Sí", "No"],
    2
  ),

  makeQuestion(
    "¿Mehdi Taremi anota o da asistencia?",
    ["Sí", "No"],
    2
  ),

  makeQuestion(
    "¿Sardar Azmoun anota en el partido?",
    ["Sí", "No"],
    2
  ),

  // ── Jugadores New Zealand ─────────────────────────────────────────────────
  makeQuestion(
    "¿Chris Wood anota en el partido?",
    ["Sí", "No"],
    2
  ),
